...
async function sendConsigneeNotification({ phone, name, awb, status }) {
  const supportPhone = process.env.NEXT_PUBLIC_SUPPORT_PHONE || '9042411159';
  const templateName = CONSIGNEE_TEMPLATES[status] || FALLBACK_TEMPLATE;

  // Fallback template needs 4 params (includes status text); specific ones need 3
  const params = templateName === FALLBACK_TEMPLATE
    ? [name, awb, status, supportPhone]
    : [name, awb, supportPhone];

  // If receiver's out of delivery template, pass the AWB as a button parameter
  const buttonParams = templateName === 'fe_rcvr_out_delivery'
    ? [awb]
    : undefined;

  return await callWhatsAppAPI({ phone, templateName, params, buttonParams });
}
...
async function sendConsignorNotification({ phone, name, awb, status, consigneeName }) {
  ...
  const supportPhone = process.env.NEXT_PUBLIC_SUPPORT_PHONE || '9042411159';
  const templateName = CONSIGNOR_TEMPLATES[status] || FALLBACK_TEMPLATE;

  // Consignor templates: name, awb, consignee_name, support_phone
  const params = templateName === FALLBACK_TEMPLATE
    ? [name, awb, status, supportPhone]
    : [name, awb, consigneeName || 'the recipient', supportPhone];
